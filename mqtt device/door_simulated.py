...
def on_message(client, userdata, message):
    ...
    if received_msg == "STOP":
        ...
        time.sleep(4)       # wait 4 seconds in order to ensure that we process every message.
...
def door_state(message):
    ...
    if people_inside < max_allowed:
        if message == "IN":
            people_inside += 1
        if message == "OUT":
            if people_inside > 0: # Bug handling for simulation purposes. make sure the counter does not go below zero. Can't be -1 person in a room.
                people_inside -= 1
    else:
        if message == "IN":
            print("Maximum number of people inside, please wait until someone leaves before entering")
        if message == "OUT":
            people_inside -= 1
    print("People inside:", people_inside)
...
try:
    ...
    client.loop_forever()            # start the event processing loop, this loop is terminated if a STOP-string is received in on_message.
